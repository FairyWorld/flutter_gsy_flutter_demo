  ...
  void _updatePhysics() {
    ...
    for (var p in particles) {
      if (p.isDead && spawnCount > 0) {
        ...
        // 稍微增加一点随机散布
        double r =
            blackEyeRadius + 2.0 + (rng.nextDouble() * rng.nextDouble() * 30.0);
    ...
    for (var p in particles) {
      if (!p.isDead) {
        ...
        if (_isSharpMode) {
          ...
          // 计算到两个“鱼头”圆心的距离平方
          double distToWhiteLobeSq =
              pow(relX, 2) + pow(relY - lobeCenterDist, 2).toDouble();
          double distToBlackLobeSq =
              pow(relX, 2) + pow(relY + lobeCenterDist, 2).toDouble();
          ...
          double distToVoid = sqrt(
              pow(p.x - voidCenterXImp1, 2) + pow(p.y - voidCenterYImp1, 2));
...
class MasterpiecePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Offset blackEyePos =
        Offset(cos(angle) * eyeOrbitRadius, sin(angle) * eyeOrbitRadius);
    Offset whiteEyePos = Offset(
        cos(angle + pi) * eyeOrbitRadius, sin(angle + pi) * eyeOrbitRadius);
