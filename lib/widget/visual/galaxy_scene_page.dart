...
class _GalaxySceneState extends State<GalaxyScene>
  ...
  final List<double> _blackHoleAngles =
      List.generate(5, (index) => Random().nextDouble() * pi);
...
class GalaxyPainter extends CustomPainter {
  ...
  void _drawActiveCore(
    ...
    Offset getTurbulence(int i, double phase, double t) {
      double speed = 2.0;
      double dx =
          0.06 * sin(t * speed + phase) + 0.03 * cos(t * speed * 2.3 + i * 0.1);
      double dy = 0.06 * cos(t * speed * 1.5 + phase) +
          0.03 * sin(t * speed * 1.9 + i * 0.1);
    ...
    void drawLobes(List<Offset> offsets, List<double> sizes,
        List<double> phases, int side) {
      ...
      for (int i = 0; i < offsets.length; i++) {
        ...
        final Paint blobPaint = Paint()
          ..shader = ui.Gradient.radial(
              pos, size, blobColors, blobStops, TileMode.clamp)
    ...
    // C. 黑洞
    final List<Color> blackHoleColors = [
      Colors.black,
    ...
    for (int i = 0; i < blackHoleAngles.length; i++) {
      ...
      final Float64List matrixStorage = (Matrix4.identity()
            ..scaleByVector3(Vector3(1.0, height / width, 1.0)))
          .storage;
      final Paint bhPaint = Paint()
        ..shader = ui.Gradient.radial(Offset.zero, width, blackHoleColors,
            blackHoleStops, TileMode.clamp, matrixStorage);

      canvas.drawOval(
          Rect.fromCenter(
              center: Offset.zero, width: width * 1.8, height: height * 1.8),
          bhPaint);
      canvas.restore();
    }
    canvas.restore();
