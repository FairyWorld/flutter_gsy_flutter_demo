...
class _SpatialGridPageState extends State<SpatialGridPage>
  ...
  void _onTick() {
    ...
    if (_isActive) {
      double damping = 0.1;
      double dx =
          _dragLagPoint.dx + (_mousePoint.dx - _dragLagPoint.dx) * damping;
      double dy =
          _dragLagPoint.dy + (_mousePoint.dy - _dragLagPoint.dy) * damping;
        ...
        builder: (context, constraints) {
          ...
          return MouseRegion(
            ...
            child: GestureDetector(
              onPanUpdate: (details) =>
                  _updateTouch(details.localPosition, size),
...
class _GridPainter extends CustomPainter {
  ...
  Offset _project3D(double x, double y, double z, Offset center, Size size) {
    ...
    return Offset(center.dx + x * size.width * 0.45 * perspective,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // --- 1. 计算网格点 (几何计算保持不变) ---
    List<List<Offset>> gridPoints =
        List.generate(rows, (i) => List.filled(cols, Offset.zero));
    // 用于存储原始未投影坐标，用于后续计算距离
    List<List<Offset>> rawPoints =
        List.generate(rows, (i) => List.filled(cols, Offset.zero));
    ...
    if (!isCyberpunkMode) {
      ...
      for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
          if (j < cols - 1) {
            canvas.drawLine(
                gridPoints[i][j], gridPoints[i][j + 1], gridCorePaint);
          }
          if (i < rows - 1) {
            canvas.drawLine(
                gridPoints[i][j], gridPoints[i + 1][j], gridCorePaint);
          }
        }
      }
    } else {
  ...
  Path _generatePath(_LightBeam beam, double startT, double endT,
      List<List<Offset>> gridPoints) {
  ...
  Offset _getPointAt(
      _LightBeam beam, double rawIdx, List<List<Offset>> gridPoints) {
