...
class DiscoSphereFinalFusionPage extends StatefulWidget {
  ...
  @override
  State<DiscoSphereFinalFusionPage> createState() =>
      _DiscoSphereFinalFusionPageState();
}
...
class _DiscoSphereFinalFusionPageState extends State<DiscoSphereFinalFusionPage>
      ...
      setState(() {
        ...
        if (_isBreathing) {
          ...
          if (_nextBreathTime <= 0) {
            ...
            _activeBurstCenter =
                _Vec3(sin(phi) * cos(theta), cos(phi), sin(phi) * sin(theta));
          }
        }
  ...
  void _initGrid() {
    ...
    List<double> rowSpeeds = List.generate(_latSegments, (rowIndex) {
      ...
      if (fromBottom == 7 || fromBottom == 9 || fromBottom == 11) {
        isFastRow = true;
      }
    ...
    for (int y = 0; y < _latSegments; y++) {
      for (int x = 0; x < _lonSegments; x++) {
        ...
        _Vec3 tumble =
            _Vec3(random.nextDouble(), random.nextDouble(), random.nextDouble())
                .normalize();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        elevation: 0,
        icon: Icon(_isPaperSpreadMode ? Icons.layers : Icons.grain,
            color: Colors.white),
        label: Text(
...
class _BreathingSpherePainter extends CustomPainter {
  ...
  double _pseudoNoise(double x, double y, double z, double t) {
    double n =
        sin(x * 5.5 + t) + sin(y * 4.5 + t * 1.2) + cos(z * 5.0 + t * 0.8);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var tile in tiles) {
      ...
      if (currentBreathIntensity > 0.0) {
        globalWobble = sin(worldCenter.y * 3.0 + time * 5.0) *
            4.0 *
        ...
        if (distToGlitch < 0.7) {
          ...
          double noise =
              _pseudoNoise(worldCenter.x, worldCenter.y, worldCenter.z, 0.0);
      ...
      if (isPaperSpreadMode) {
        ...
        if (shouldTumble) {
          ...
          sT = sin(tumbleAngle);
          oneCT = 1 - cT;
          ux = tile.tumbleAxis.x;
          uy = tile.tumbleAxis.y;
          uz = tile.tumbleAxis.z;
        }
        for (var vBase in tile.vertices) {
          _Vec3 vRot = _rotate(vBase, cosX, sinX, cosY, sinY);
          _Vec3 vLocal = (vRot - worldCenter) * radius;
          if (shouldTumble) {
            ...
            double rx = ux * dot * oneCT +
                vLocal.x * cT +
                (uy * vLocal.z - uz * vLocal.y) * sT;
            double ry = uy * dot * oneCT +
                vLocal.y * cT +
                (uz * vLocal.x - ux * vLocal.z) * sT;
            double rz = uz * dot * oneCT +
                vLocal.z * cT +
                (ux * vLocal.y - uy * vLocal.x) * sT;
